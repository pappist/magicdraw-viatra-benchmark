...
cd ${WORKSPACE}/com.incquerylabs.instaschema.test/results
...
for i in $(ls -d */); do
  ...
  if [ "warmup/" == ${i} ]; then
	...
	python3 ${WORKSPACE}/mondo-sam/reporting/convert_results.py --source ${WORKSPACE}/com.incquerylabs.instaschema.test/results/${i} \
